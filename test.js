...
pdf(dataBuffer).then(function (data) {
  ...
  for (i = 0; i <= newArray.length; i++) {
    ...
    if (newArray[i] === "Average" && newArray[i + 2] === "age" && count === 4) {
      ...
      const age = `${newArray[i + 3]} ${newArray[i + 4]} ${newArray[i + 5]} ${
        newArray[i + 6]
      }`;
      ...
      accountsRaw.forEach((account) => {
        ...
        account.forEach((cell, index) => {
          ...
          if (cell.includes("type")) {
            const endOfTypeIndex = account.indexOf("StatusOpen");
            const type = account
    ...
    if (newArray[i] === "Inquiries" && count === 7) {
      ...
      inquiriesFinal.forEach((inquiry) => {
        ...
        if (differenceInMonths(today, inquiryDate) <= 3) {
          threeMonths++;
        }
        if (
          differenceInMonths(today, inquiryDate) > 3 &&
          differenceInMonths(today, inquiryDate) <= 6
        ) {
          sixMonths++;
        }
      });
